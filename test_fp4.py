...
def mxfp4_dot_scaled_kernel(
    ...
    sB_ptr,
    stride_am,
    stride_ak2,  # A_packed: [M, K/2]
    stride_bk2,
    stride_bn,  # B_packed: [K/2, N]
    stride_cm,
    stride_cn,  # C: [M, N]
...
def run_mxfp4_dot_scaled_kernel(a_codes: torch.Tensor, sA_grouped: torch.Tensor, b_codes: torch.Tensor,
    ...
    mxfp4_dot_scaled_kernel[grid](a_codes, b_codes, c, M, N, K, sA_grouped, sB_grouped, (K // 2), 1, N, 1, N, 1,
...
def test_mxfp4_matmul(M: int, N: int, K: int):
    ...
    # Compute reference
    c_ref = mxfp4_scaled_reference_grouped(a_packed=a_codes, b_packed=b_codes, sA_grouped=sA, sB_grouped=sB, M=M, N=N,
                                           K=K, GROUP_K=GROUP_K)
